...
script_version="2.0"
ramdisk_cache_version="2.0-a4.13"
...
mk_bruteforce_ramdisk() {
    ...
    if [[ "$is_64" == "true" ]]; then
        ...
        if [[ "$device" == "iPad1,1" ]]; then
            ...
            run_checked "Install iPad1,1 rc.boot" \
                cp ../../../resources/rc.boot.ipad ramdisk_mountpoint/private/etc/rc.boot
            chmod +x ramdisk_mountpoint/private/etc/rc.boot ||
                die "Failed to make iPad1,1 rc.boot executable."
            rm -f ramdisk_mountpoint/iwannabrute.delay
            mkfifo ramdisk_mountpoint/iwannabrute.delay ||
                die "Failed to create the iPad1,1 delay fifo."
        fi
...
get_device_info() {
    ...
    case $deviceid in
        ...
        "iPad1,1") device_name="iPad 1" default_version="5.1.1" pwnder="ipwnder32" is_a4=true ramdisk_cache_version="2.0-a4.13" ;;
